...
function game(){
  ...
  if(gameConfig.status==='start'){
    ...
    textSize(40);
    textAlign(CENTER);
    text("Pressione o botão para iniciar o jogo", gameConfig.screenX/2, gameConfig.screenY/2);
    textSize(40);
...
function changeGameStatud(character){
  if(gameConfig.status==="start" && statusDoJogo == "start" && noseX != 0) {
    world_start.play();
